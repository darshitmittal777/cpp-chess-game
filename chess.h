...
struct Position {
    ...
    static Position fromAlgebraic(const std::string& algebraic); //static -> can call without making an instance
...
struct Move {
    ...
    //Below are 2 constructors for Move: Constructor Overloading
    Move(Position f, Position t) : from(f), to(t) {} // Normal move without promotion
    Move(Position f, Position t, PieceType promo) : from(f), to(t), promotionPiece(promo) {} // Move with promotion
};
...
public:
    ...
    //virtual methods, can't call the methods directly; have to call via derived classes
...
public:
    ...
    // override: means that this function overrides a virtual function from base class
